                                         ...
                                         double& height)
{
    ...
    //TODO
    //根据函数进行投影．计算height，即ppt中的I(x) 曲面重建
    for (int i = 0; i < nearPoints.size(); i++) {
        Eigen::Vector2d delta_x = x - nearPoints[i];
        double weight_i = exp(-delta_x.squaredNorm()/(m_r*m_r));
        weightSum += weight_i;
        projSum += weight_i*delta_x.transpose()*nearNormals[i];
    }
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    for(int i = 0; i < nearPoints.size();i++){
        sigma += (nearPoints[i] - u)*(nearPoints[i] - u).transpose();
    }
    sigma /= nearPoints.size();

    //sigma最小特征值对应的特征向量是法向量
