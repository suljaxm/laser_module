            ...
            int& beam_number)
    {
       //TODO
        ...
        for(int i = 0; i< beam_number;i++){
            //角度插值
            mid_angle =  tf::getYaw(frame_start_pose.getRotation().slerp(frame_end_pose.getRotation(), beam_step * i));
            ...
            if( tfFuzzyZero(ranges[startIndex + i]) == false){
                ...
                //把在odom坐标系中的激光数据点转换到基础坐标系
                double base_s, base_c;
                base_s = sin(base_Yaw);
                base_c = cos(base_Yaw);

                double tmp_x,tmp_y;
                tmp_x =  mid_point.x()*base_c  + mid_point.y()*base_s - base_pos.x()*base_c - base_pos.y()*base_s;
                tmp_y = -mid_point.x()*base_s  + mid_point.y()*base_c  + base_pos.x()*base_s - base_pos.y()*base_c;
                mid_point.setValue(tmp_x,tmp_y,0);

                //然后计算以起始坐标为起点的 dist_angle
                double dx,dy;
                dx = (mid_point.x());
                dy = (mid_point.y());
            ...
            //如果等于无穷,则随便计算一下角度
                ...
                lidar_angle = tfNormalizeAngle(tmp_angle - start_Yaw);
